...
-- I have 1  automatic index  that is emp_id  after crate emp_salary_idx we have 2 automatic index

--2.2
CREATE INDEX emp_dept_idx ON employees(dept_id);
SELECT * FROM employees WHERE dept_id = 101;

-- speeds up JOIN operations with the parent table
-- speeds up queries that filter by dept_id
...
--place NULL values at the top of the order
...
--compute LOWER(emp_name) for every row
...
-- many INSERT/DELETE operations caused index bloat
...
-- are faster, because they cover fewer rows

-- PART 8.3
EXPLAIN SELECT * FROM employees WHERE salary > 52000;

-- Index Scan → the query uses an index → performance optimized
...
--Reason: holds more data → more pages → more space.
...
-- 1 B-tree
-- 2 When a column is used in WHERE, JOIN, or ORDER BY
--3 When the table is small or the column has low selectivity
-- 4  Indexes are updated, which slows down write operations
-- 5 Use EXPLAIN to see whether an Index Scan is used.